...
const VoterForm = ({
  ...
  const { errors, hasErrors, setErrors, handleChange } =
    useValidatedForm<Voter>(insertVoterParams);
  ...
  const onSuccess = (
    action: Action,
    data?: { error: string; values: Voter },
  ) => {
  ...
  const handleSubmit = async (data: FormData) => {
    ...
    try {
      startMutation(async () => {
        ...
        const error = editing
          ? await updateVoterAction({ ...values, id: voter.id })
          : await createVoterAction(values);
    ...
    <form action={handleSubmit} onChange={handleChange} className={"space-y-8"}>
      {/* Schema fields start */}
        ...
        <Label
          className={cn(
            "mb-2 inline-block",
            errors?.school ? "text-destructive" : "",
          )}
        >
          School
        </Label>
        <Input
          type="text"
          name="school"
          className={cn(errors?.school ? "ring ring-destructive" : "")}
          defaultValue={voter?.school ?? ""}
        />
        {errors?.school ? (
          <p className="text-xs text-destructive mt-2">{errors.school[0]}</p>
        ) : (
          <div className="h-6" />
        )}
      </div>
        ...
        <Label
          className={cn(
            "mb-2 inline-block",
            errors?.township ? "text-destructive" : "",
          )}
        >
          Township
        </Label>
        <Input
          type="text"
          name="township"
          className={cn(errors?.township ? "ring ring-destructive" : "")}
          defaultValue={voter?.township ?? ""}
        />
        {errors?.township ? (
          <p className="text-xs text-destructive mt-2">{errors.township[0]}</p>
        ) : (
          <div className="h-6" />
        )}
      </div>
      ...
      {leaderId ? null : <div>
        <Label
          className={cn(
            "mb-2 inline-block",
            errors?.leaderId ? "text-destructive" : "",
          )}
        >
          Leader
        </Label>
        <Select defaultValue={voter?.leaderId} name="leaderId">
          <SelectTrigger
            className={cn(errors?.leaderId ? "ring ring-destructive" : "")}
          >
          ...
          {leaders?.map((leader) => (
            <SelectItem key={leader.id} value={leader.id.toString()}>
              {leader.id}{/* TODO: Replace with a field from the leader model */}
            </SelectItem>
           ))}
          </SelectContent>
        </Select>
        {errors?.leaderId ? (
          <p className="text-xs text-destructive mt-2">{errors.leaderId[0]}</p>
        ) : (
          <div className="h-6" />
        )}
      </div> }
